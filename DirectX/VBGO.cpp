...
VBGO::~VBGO()
{
	...
	DESTROY(pVertexShader);
	DESTROY(pVertexLayout);
	DESTROY(pPixelShader);
	DESTROY(pTextureRV);
	DESTROY(pConstantBuffer);
	//if (m_pCB) delete m_pCB; delete this where created as there will know its type
	DESTROY(pSampler);
	DESTROY(pRasterState);
}
...
{
	//set raster state
	ID3D11RasterizerState* useRasterS = pRasterState ? pRasterState : s_pRasterState;
	...
	ID3D11Buffer* useCB = pConstantBuffer ? pConstantBuffer : s_pConstantBuffer;
	void* useCBData = pCB ? pCB : s_pCB;
	...
	ID3D11InputLayout* useLayout = pVertexLayout ? pVertexLayout : s_pVertexLayout;
	_DD->m_pd3dImmediateContext->IASetInputLayout(useLayout);

	//set Vertex Shader
	ID3D11VertexShader* useVS = pVertexShader ? pVertexShader : s_pVertexShader;
	_DD->m_pd3dImmediateContext->VSSetShader(useVS, NULL, 0);

	//set Pixel Shader
	ID3D11PixelShader* usePS = pPixelShader ? pPixelShader : s_pPixelShader;
	_DD->m_pd3dImmediateContext->PSSetShader(usePS, NULL, 0);

	//set Texture
	ID3D11ShaderResourceView* useTex = pTextureRV ? pTextureRV : s_pTextureRV;
	_DD->m_pd3dImmediateContext->PSSetShaderResources(0, 1, &useTex);

	//set sampler
	ID3D11SamplerState* useSample = pSampler ? pSampler : s_pSampler;
...
void VBGO::Init(ID3D11Device* _GD)
{
	...
	HRESULT hr = CompileShaderFromFile(L"shaders.shader", "VS", "vs_4_0_level_9_1", &pVertexShaderBuffer);
	_GD->CreateVertexShader(pVertexShaderBuffer->GetBufferPointer(), pVertexShaderBuffer->GetBufferSize(), NULL, &s_pVertexShader);

	//default pixelshader
	ID3DBlob* pPixelShaderBuffer = NULL;
	hr = CompileShaderFromFile(L"shaders.shader", "PS", "ps_4_0_level_9_1", &pPixelShaderBuffer);
	_GD->CreatePixelShader(pPixelShaderBuffer->GetBufferPointer(), pPixelShaderBuffer->GetBufferSize(), NULL, &s_pPixelShader);

	//default vertex layout
	_GD->CreateInputLayout(vertexLayout, sizeof(VERTEX), pVertexShaderBuffer->GetBufferPointer(),
